...
**Keywords:** mcp-server, model-context-protocol, nova-by-civai, remote-mcp, streamable-http, civai, cursor, claude-desktop, telegram, messaging, mtproto
...
| Resource | URL |
|----------|-----|
| MCP endpoint | https://nova.civai.co/mcp/agents/telegram-agent |
| Agent page | https://nova.civai.co/agent/telegram-agent |
| API keys | https://nova.civai.co/api-keys |
| Official Registry | `co.civai.nova/telegram-agent` |
...
Local test: copy this folder to `~/.cursor/plugins/local/telegram-agent-by-nova`, reload the window, set `NOVA_API_KEY`, verify MCP tools load.
...
```bash
claude mcp add --transport http telegram-agent-by-nova \
...
- **Tools:** 16 (bundled catalog)

## About Nova by CIVAI

**Nova by CIVAI** is a platform for running, building, and deploying AI agents. This MCP server exposes **Telegram Agent** tools to external MCP hosts without sharing upstream credentials.
